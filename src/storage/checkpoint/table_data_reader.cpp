...
void TableDataReader::ReadTableData() {
	...
	for (idx_t col = 0; col < columns.size(); col++) {
		...
		for (idx_t data_ptr = 0; data_ptr < data_pointer_count; data_ptr++) {
			...
			data_pointer.compressed_count = reader.Read<idx_t>();
			...
			auto segment = make_unique<PersistentSegment>(
			    ...
			    data_pointer.row_start, data_pointer.tuple_count, data_pointer.min_stats, data_pointer.max_stats,manager.database.config.enable_rle,data_pointer.compressed_count);
