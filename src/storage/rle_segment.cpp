...
RLESegment::update_info_fetch_function_t RLESegment::GetUpdateInfoFetchFunction(PhysicalType type) {
	...
	default:
		throw NotImplementedException("Unimplemented type for RLE segment");
	}
}
...
void Select_RLE(SelectionVector &sel, Vector &result, unsigned char *source, nullmask_t *source_nullmask,
	...
	static void templated_select_rle_operation_between(
	    SelectionVector & sel, Vector & result, PhysicalType type, unsigned char *source, nullmask_t *source_mask,
	...
	void RLESegment::Select(ColumnScanState & state, Vector & result, SelectionVector & sel,
		...
		if (tableFilter.size() == 1) {
			...
			case ExpressionType::COMPARE_GREATERTHAN: {
				templated_select_rle_operation<GreaterThan>(sel, result, state.current->type, source_data,
	...
	static void update_loop_no_null_rle(T * __restrict undo_data, T * __restrict base_data, T * __restrict new_data,
	                                    idx_t count, sel_t * __restrict base_sel, T * __restrict min,
	                                    T * __restrict max) {
		for (idx_t i = 0; i < count; i++) {
			// first move the base data into the undo buffer info
			undo_data[i] = base_data[base_sel[i]];
	...
	void RLESegment::FetchBaseData(ColumnScanState & state, idx_t vector_index, Vector & result) {
		...
		DecompressRLE(data, result, type, count);
	}
	...
	template <class T> static void rle_decompress(data_ptr_t source, Vector & target, idx_t count) {
	...
	void RLESegment::DecompressRLE(data_ptr_t source, Vector & target, PhysicalType type, idx_t count) {
		switch (type) {
		case PhysicalType::INT32:
